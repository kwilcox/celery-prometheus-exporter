...
class TaskMonitoringThread(threading.Thread):
    """
    TaskMonitoringThread is the thread that will collect the data that is later
...
class WorkerMonitoringThread(threading.Thread):
    ...
    def _monitor(self):
        while True:
            workers = self._app.control.ping(timeout=5)
...
def main():
    ...
    celery_app = celery.Celery(broker=opts.broker)
    t = TaskMonitoringThread(app=celery_app)
